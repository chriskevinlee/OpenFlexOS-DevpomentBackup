...
if [[ $wm_dir == "openbox" ]]; then
    ...
    for openboxfile in {autostart,menu.xml,snap.sh,environment,rc.xml}; do
        ln -sf /etc/openflexos/home/users/config/openbox/$openboxfile "/etc/skel/.config/openbox/$openboxfile"
    done
    ...
    for openboxtint2 in tint2rc; do
        ln -sf /etc/openflexos/home/users/config/openbox/tint2/$openboxtint2 "/etc/skel/.config/openbox/tint2/$openboxtint2"
    done

    # Rofi
    for openboxrofi in {config.rasi,Generate_Rofi_theme.sh,theme.rasi}; do
        ln -sf /etc/openflexos/home/users/config/openbox/rofi/$openboxrofi "/etc/skel/.config/openbox/rofi/$openboxrofi"
    done

    # Oh My Posh
    for ohmyposh in base.toml; do
        ln -sf /etc/openflexos/home/users/config/ohmyposh/$ohmyposh "/etc/skel/.config/ohmyposh/$ohmyposh"
    done

    # SXIV Key Handler
    for sxiv in key-handler; do
        ln -sf /etc/openflexos/home/users/config/sxiv/exec/$sxiv "/etc/skel/.config/sxiv/exec/$sxiv"
    done

    # web_bookmarks
    for webbookmarks in sites.txt; do
        ln -sf /etc/openflexos/home/users/config/web_bookmarks/$webbookmarks "/etc/skel/.config/web_bookmarks/$webbookmarks"
    done

    # Dunst
    for dunst in dunstrc; do
        ln -sf /etc/openflexos/home/users/config/dunst/$dunst "/etc/skel/.config/dunst/$dunst"
    done

    # Alacritty
    for alacritty in alacritty.toml; do
        ln -sf /etc/openflexos/home/users/config/alacritty/$alacritty "/etc/skel/.config/alacritty/$alacritty"
    done

    # GTK 3
    for gtk3 in gtk.css; do
        ln -sf /etc/openflexos/home/users/config/gtk-3.0/$gtk3 "/etc/skel/.config/gtk-3.0/$gtk3"
    done

    # GTK 4
    for gtk4 in gtk.css; do
        ln -sf /etc/openflexos/home/users/config/gtk-4.0/$gtk4 "/etc/skel/.config/gtk-4.0/$gtk4"
    done

    # MyThemes
    for mythemes in MyThemes.json; do
        ln -sf /etc/openflexos/home/users/config/MyThemes/$mythemes "/etc/skel/.config/MyThemes/$mythemes"
    done

    # Picom
    for picom in picom.conf; do
        ln -sf /etc/openflexos/home/users/config/picom/$picom "/etc/skel/.config/picom/$picom"
    done

    # Qt5
    for qt5 in qt5.conf; do
        ln -sf /etc/openflexos/home/users/config/qt5ct/$qt5 "/etc/skel/.config/qt5ct/$qt5"
    done

    # Qt6
    for qt6 in qt6.conf; do
        ln -sf /etc/openflexos/home/users/config/qt6ct/$qt6 "/etc/skel/.config/qt6ct/$qt6"
    done

    # Dotfiles
    for userhomedots in {dot.bashrc,dot.gtkrc-2.0,dot.zshrc,dot.xscreensaver}; do
        ln -sf /etc/openflexos/home/users/$userhomedots "/etc/skel/.${userhomedots#dot.}"
    done

    for wallpapers in /etc/openflexos/home/users/config/wallpapers/wallpaper_cave_nature/*; do
...
if [[ $wm_dir == "qtile" ]]; then
    ...
    for qtileconfig in {config.py,OpenFlexOS_AutoStart.sh}; do
        ln -sf /etc/openflexos/home/users/config/qtile/$qtileconfig "/etc/skel/.config/qtile/$qtileconfig"
    done
    ...
    for qtilerofi in {config.rasi,Generate_Rofi_theme.sh,theme.rasi}; do
        ln -sf /etc/openflexos/home/users/config/qtile/rofi/$qtilerofi "/etc/skel/.config/qtile/rofi/$qtilerofi"
    done
    ...
    for qtilesounds in {ambient-piano-logo-165357.mp3,cozy-weaves-soft-logo-176378.mp3,error-83494.mp3,game-bonus-144751.mp3,introduction-sound-201413.mp3,lovelyboot1-103697.mp3,machine-error-by-prettysleepy-art-12669.mp3,marimba-win-f-2-209688.mp3,retro-audio-logo-94648.mp3}; do
        ln -sf /etc/openflexos/home/users/config/qtile/sounds/$qtilesounds "/etc/skel/.config/qtile/sounds/$qtilesounds"
    done
